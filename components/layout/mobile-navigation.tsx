...
export function MobileNavigation() {
  ...
  return (
    <div className="lg:hidden">
      <button ref={openButtonRef} type="button" onClick={() => setIsOpen(true)} className="inline-flex border border-[var(--line)] p-3 text-[var(--cream)]" aria-label="Navigation öffnen" aria-controls="mobile-navigation" aria-expanded={isOpen}>
      ...
      <nav id="mobile-navigation" aria-label="Mobile Navigation" aria-hidden={!isOpen} inert={!isOpen} style={{ backgroundColor: "#141210" }} className={`fixed inset-y-0 left-0 z-[70] isolate flex w-[min(88vw,360px)] flex-col border-r border-[var(--line)] p-6 shadow-2xl transition-transform ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={closeNavigation} className="border border-[var(--line)] p-2" aria-label="Navigation schliessen"><X className="h-5 w-5" /></button>
        </div>
        <div className="mt-10 grid gap-2">
          ...
          <Link href="/search" onClick={closeNavigation} className="site-button site-button--secondary mt-3 justify-start"><Search className="h-4 w-4" /> Produkte suchen</Link>
        </div>
      </nav>
    </div>
  );
